    ...
    for (i = adventurer; i <= treasure_map; i++) // loop all cards
    {
        for (j = 0; j < 10; j++) // loop chosen cards
        {
            if (kingdomCards[j] == i) {
                ...
                break;
            } else // card is not in the set choosen for the
            // game
            {
...
int endTurn(struct gameState * state) {
    ...
    if (currentPlayer < (state - > numPlayers - 1)) {
        ...
        state - > whoseTurn = 0; // Max player has been reached, loop back
        // around to player 1
    }
...
int drawCard(int player, struct gameState * state) {
    ...
    if (state - > deckCount[player] <= 0) { // Deck is empty
        ...
        // Shufffle the deck
        shuffle(player, state); // Shuffle the deck up and make it so that 
        // we can draw
        ...
        // Step 2 Draw Card
        count = state - > handCount[player]; // Get current player's
        // hand count
        ...
        deckCounter = state - > deckCount[player]; // Create a holder for the 
        // deck count

        if (deckCounter == 0)
            return -1;

        state - > hand[player][count] = state - > deck[player][deckCounter - 1]; // Add 
        // card 
        // to 
        // hand
        state - > deckCount[player]--;
        ...
        int count = state - > handCount[player]; // Get
        // current 
        // hand
        // count
        // for
        ...
        deckCounter = state - > deckCount[player]; // Create holder for the
        // deck count
        state - > hand[player][count] = state - > deck[player][deckCounter - 1]; // Add 
        // card 
        // to 
...
int cardEffect(int card, int choice1, int choice2, int choice3,
    ...
    switch (card) {
        ...
        while (drawntreasure < 2) {
            if (state - > deckCount[currentPlayer] < 1) { // if the deck is
                // empty we need
                // to shuffle
                // discard and add 
                // to deck
            ...
            cardDrawn = state - > hand[currentPlayer][state - > handCount[currentPlayer] - 1]; // top 
            // card 
            // of 
            // hand 
            // is 
            ...
            if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
                ...
                state - > handCount[currentPlayer]--; // this should
                // just remove the 
                // top card (the
                // most recently
                // drawn one).
        ...
        // Backup hand
        for (i = 0; i <= state - > handCount[currentPlayer]; i++) {
            temphand[i] = state - > hand[currentPlayer][i]; // Backup
            // card
        ...
        while (x == 1) { // Buy one card
            if (supplyCount(choice1, state) <= 0) {
                ...
                gainCard(choice1, state, 0, currentPlayer); // Gain
                // the
                // card
        ...
        case mine:
        j = state - > hand[currentPlayer][choice1]; // store card we
        // will trash
    ...
    case baron:
        ...
        if (choice1 > 0) { // Boolean true or going to discard an
            // estate
            int p = 0; // Iterator for hand!
            int card_not_discarded = 1; // Flag for
            // discard set!
            while (card_not_discarded) {
                if (state - > hand[currentPlayer][p] == estate) { // Found
                    // an
                    // estate
                    // card!
                    state - > coins += 4; // Add 4 coins to the amount of
                    // coins
                    state - > discard[currentPlayer][state - >
                        discardCount[currentPlayer]
                        ] =
                    ...
                    if (supplyCount(estate, state) > 0) {
                        ...
                        state - > supplyCount[estate]--; // Decrement
                        // estates
                        if (supplyCount(estate, state) == 0) {
                            isGameOver(state);
                        }
            ...
            if (supplyCount(estate, state) > 0) {
                gainCard(estate, state, 0, currentPlayer); // Gain an 
                // estate
        ...
        } else if (choice2) // discard hand, redraw 4, other players
        ...
        if ((state - > discardCount[nextPlayer] +
            ...
            if (state - > deckCount[nextPlayer] == 0) {
                for (i = 0; i < state - > discardCount[nextPlayer]; i++) {
                    state - > deck[nextPlayer][i] = state - > discard[nextPlayer][i]; // Move 
                    // to 
                    // deck
                    state - > deckCount[nextPlayer]++;
        ...
        if (tributeRevealedCards[0] == tributeRevealedCards[1]) { // If 
            // we 
            // have 
            // a 
            // duplicate 
        ...
        for (i = 0; i <= 2; i++) {
            if (tributeRevealedCards[i] == copper || tributeRevealedCards[i] == silver || tributeRevealedCards[i] == gold) { // Treasure 
                // cards
                state - > coins += 2;
            } else if (tributeRevealedCards[i] == estate || tributeRevealedCards[i] == duchy || tributeRevealedCards[i] == province || tributeRevealedCards[i] == gardens || tributeRevealedCards[i] == great_hall) { // Victory 
                // Card 
                // Found
                drawCard(currentPlayer, state);
                drawCard(currentPlayer, state);
        ...
        case ambassador:
        j = 0; // used to check if player has enough
        // cards to discard
    ...
    for (i = 0; i < state - > numPlayers; i++) {
        if (i != currentPlayer) {
            ...
            state - > discardCount[i]++;
                state - > deck[i][state - > deckCount[i]--] = curse; // Top
                // card
                // now a
                // curse
            }
...
int discardCard(int handPos, int currentPlayer, struct gameState * state,
    ...
    if (handPos == (state - > handCount[currentPlayer] - 1)) // last
    // card in 
    // hand
    // array
    // is
    // played
    {
        ...
        state - > handCount[currentPlayer]--;
    } else if (state - > handCount[currentPlayer] == 1) // only one card
    // in hand
    {
        // reduce number of cards in hand
        state - > handCount[currentPlayer]--;
