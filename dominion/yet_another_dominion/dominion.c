...
void card_councilRoom(struct gameState *state, int currentPlayer, int handPos){
    // Add four cards
    for(int i = 0; i < 4; i++){
        drawCard(currentPlayer, state);
    }
...
void card_feast(struct gameState *state, int currentPlayer, int choice1){
    int tempHand[MAX_HAND];

    // Gain a card with a cost of up to 5
    // Backup hand?
...
int card_mine(struct gameState *state, int currentPlayer, int handPos, int choice1, int choice2){
    int j;
...
int card_remodel(struct gameState *state, int currentPlayer, int handPos, int choice1, int choice2){
    int j;
    ...
    //discard trashed card
    for (int i = 0; i < state->handCount[currentPlayer] - 1; i++){
        if (state->hand[currentPlayer][i] == j){
            discardCard(i, currentPlayer, state, 0);
            break;
        }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus){
    ...
    switch(card){
        case adventurer:
            card_adventurer(state, currentPlayer);
            return 0;

        case council_room:
            card_councilRoom(state, currentPlayer, handPos);
            return 0;

        case feast:
            card_feast(state, currentPlayer, choice1);
            return 0;
        ...
        case mine:
            return card_mine(state, currentPlayer, handPos, choice1, choice2);

        case remodel:
            return card_remodel(state, currentPlayer, handPos, choice1, choice2);
